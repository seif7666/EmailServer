...
<script>
  ...
  export default {
    props: ["id","head", "email", "date", "body","priority" ,"isSent","attachments" , "draft"],
    name: "mail",
    data(){
      return{
        marked:false,
    ...
    methods:{
      ...
      checked(){
        ...
        //Here we download
        const url = `${BACKEND_URL}download?id=${this.id}&type=${typeOfMail}&name=${array[array.length-1]}`;
        console.log(url);
        window.open(url);
      },
    ...
    mounted(){
      ...
      if(this.isSent === true){ ///We show sender
        console.log("Entered")
